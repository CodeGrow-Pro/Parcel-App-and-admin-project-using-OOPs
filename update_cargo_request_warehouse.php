                        ...
                        <div class="card">
                           <div class="card-body">
                              <?php 
                                 if(isset($_GET['id']))
                                 {
                                 	$data = $lundry->query("select * from tbl_request_warehouse where id=".$_GET['id']."")->fetch_assoc();
                                 	?>
                              <h4 class="card-title mb-4">Edit </h4>
                              <form method="post" enctype="multipart/form-data">
                                 <div class="form-group">
                                    <label> location</label>
                                    <input type="text" class="form-control" placeholder="Enter Cargo Owner Name" value="<?php echo $data['location'];?>" name="cname" required="">
                                 </div>
                                <div class="form-group">
                                    <label> Status</label>
                                 ...
                                 <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" placeholder="Enter Cargo Owner ID Proof" value="<?php echo $data['description'];?>" name="desc" required="">
                                 </div>
                                    <div class="form-group">
                                    <label>Budget</label>
      ...
      <?php 
        if(isset($_POST['edit_cargoowner']))
        {
          ...
          $target_file2 = $target_dir . basename($newfilename2);
              ...
              $table="tbl_request_warehouse";
              $field = array('location'=>$cname,'flammable'=>$idno,'description'=>$desc,'budget'=>$budget,'size'=>$size,'status'=>$okey);
              ...
              <script>
                iziToast.success({
                    title: ' Cargo Section!!',
                    message: 'Request warehouse Update Successfully!!',
                    position: 'topRight'
                  });
                      setTimeout(function(){ 
                  window.location.href="list_cargo_request_warehouse.php"},3000);
              </script>
              <?php 
              }else{ ?>
                   <script>
                iziToast.error({
                    title: ' Cargo Section!!',
                    message: 'Request warehouse Update failed!!',
                    position: 'topRight'
                  });
                      setTimeout(function(){ 
                  window.location.href="update_cargo_request_warehouse.php"},3000);
              </script>
                 <?php 
              }
            }
