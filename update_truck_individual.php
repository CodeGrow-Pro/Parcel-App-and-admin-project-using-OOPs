...
<div class="row">
				...
				if(isset($_GET['id']))
				{
					$data = $lundry->query("select * from tbl_truck_individual where id=".$_GET['id']."")->fetch_assoc();
					?>
					<h4 class="card-title mb-4">Edit Truck Individual </h4>
...
$table="tbl_truck_individual";
  $field = array('fname'=>$reg,'tin_no'=>$certi,'certi_image'=>$target_file,'status'=>$okey);
...
<script>
 iziToast.success({
    ...
    position: 'topRight'
				 ...
				 setTimeout(function(){ 
	 window.location.href="list_truck_individual.php"},3000);
...
<?php 
	...
	else 
	{
		...
		$table="tbl_truck_individual";
  $field = array('fname'=>$reg,'tin_no'=>$certi,'status'=>$okey,);
  $where = "where id=".$_GET['id']."";
$h = new Laundrore();
	  $check = $h->lundryupdateData($field,$table,$where);
if($check == 1)
