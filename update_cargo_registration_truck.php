   ...
   <body>
                        ...
                        <div class="card">
                           <div class="card-body">
                              <?php 
                                 if(isset($_GET['id']))
                                 {
                                 	$data = $lundry->query("select * from tbl_regestration_truck_owner where id=".$_GET['id']."")->fetch_assoc();
                                 	?>
                              ...
                              <form method="post" enctype="multipart/form-data">
                                 <div class="form-group">
                                   ...
                                   <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" placeholder="Enter location" value="<?php echo $data['destination'];?>" name="destination" required="">
                                 </div>
                            <div class="form-group">
                                 ...
                                 <div class="form-group">
                                  ...
                                  <div class="form-group">
                                    <label>Warehouse Need</label>
                                    <select name="need" class="form-control" required>
                                       <option value="1" <?php if($data['warehouseneed'] == 1){echo 'selected';}?>>Yes</option>
                                       <option value="0" <?php if($data['warehouseneed'] == 0){echo 'selected';}?>>No</option>
                                    </select>
                                 </div>
                                    <div class="form-group">
                                    <label>Budget</label>
                                    ...
                                    <input type="text" class="form-control" placeholder="Enter Required size" value="<?php echo $data['numtruck'];?>" name="size" required="">
                                 </div>
                                 <div class="form-group">
                                    <label> Status</label>
      ...
      <?php 
        if(isset($_POST['edit_cargoowner']))
        {
         	...
         	$destination = mysqli_real_escape_string($lundry,$_POST['destination']);
         	$idno = mysqli_real_escape_string($lundry,$_POST['flam']);
         	$desc = mysqli_real_escape_string($lundry,$_POST['desc']);
         	$need = mysqli_real_escape_string($lundry,$_POST['need']);
          ...
          $target_file2 = $target_dir . basename($newfilename2);
              ...
              $table="tbl_regestration_truck_owner";
              $field = array('location'=>$cname,'destination'=>$destination,'tcargo'=>$idno,'description'=>$desc,'warehouseneed'=>$need,'budget'=>$budget,'numtruck'=>$size,'status'=>$okey);
              ...
              <script>
                iziToast.success({
                    title: ' Cargo Section!!',
                    message: 'Registration truck Update Successfully!!',
                    position: 'topRight'
                  });
                      setTimeout(function(){ 
                  window.location.href="list_cargo_truck_regestration.php"},3000);
              </script>
              <?php 
              }else{ ?>
                   <script>
                iziToast.error({
                    title: ' Cargo Section!!',
                    message: 'Registration truck Update failed!!',
                    position: 'topRight'
                  });
                      setTimeout(function(){ 
                  window.location.href="update_cargo_registration_truck.php"},3000);
              </script>
                 <?php 
              }
            }
